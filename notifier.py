...
def notify_changes_enhanced(opened: List[Dict], closed: List[Dict], updated: List[Dict], 
    ...
    try:
        ...
        if total_jobs <= max_individual_messages:
            ...
            logger.info(f"Too many changes ({total_jobs}), sending grouped notifications")
        ...
        logger.info(f"Successfully sent notifications for {total_jobs} job changes")
...
def send_test_notification(telegram_cfg: Dict) -> bool:
    ...
    if not bot_token or not chat_id:
        logger.error("Missing Telegram credentials for test")
        return False
    ...
    try:
        send_telegram_message(test_message, bot_token, chat_id)
